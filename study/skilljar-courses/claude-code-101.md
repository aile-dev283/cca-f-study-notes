...
1. Claude Code に prompt を入力する。
2. Claude は model とやり取りすることで必要な context を集め、model は text または Claude Code が実行できる tool call を返す。
3. アクションを起こす ── たとえば file を編集したり command を実行したりする。
4. 結果を検証し、それがあなたの prompt が目指したことを達成しているかどうかを判断する。
5. 達成していれば、Claude は終了して次の prompt を待つ。達成していなければ、ループに戻り、結果が完全で検証可能になるまで再試行する。
