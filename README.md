...
**スキルは「作成」と「出題」の2プロセスに分かれている**（呼び出すスキルは `cca-f-trainer` 1つだけ）。
問題生成は重いため、毎回の出題のたびに走らせない設計になっている。

- **作成プロセス**: コーパスから問題を生成し保存する。60問の模試なども事前に作り置きできる。
- **出題プロセス**: 既存バンクを読んで出題・採点し、弱点レポートを出す。生成しないので**即座に始まる**。

起動時に「問題を解く／問題を作成する」を選ぶだけで、適切なプロセスへ自動で振り分けられる（進行中の演習があれば自動で再開）。

**特徴:**

- 出題は公式どおりシナリオベース（6原型）。難易度は設計判断を問う L3 を主軸。
- 生成時に**正解キーのセルフチェック**（`rationale` を伏せてコーパスから正解を再導出）を行い、自作問題の「正解が間違っている」リスクを低減。
- ディストラクタは「もっともらしいアンチパターン」を強制（頻出トラップを含む）。
- カンニング防止のため、バンク内でも問題ファイル（`quiz-questions.json`）と正解ファイル（`answer-key.json`）を分離。
- 採点後、ドメイン別正答率・苦手サブドメイン・**リポジトリ内の復習先ファイルパス**・次回推奨を1枚のレポートにまとめる。
- セッションを跨いで弱点を蓄積し、**弱点ターゲットの追加作成**や**弱点優先の出題順**で間隔反復的に最適化される。
...
4. **問題を作成する（初回・追加時）**
  - **問題数**（10/20/60フル模試）・**focus**（ドメイン横断／特定ドメイン集中／弱点ターゲット）を選ぶ
  - 問題は**本番と同じ英語（canonical）で生成**され `banks/<バンク名>/` に保存される（例 `mock-60-2026-06-20`）。言語は解く時に選ぶのでここでは聞かれない。60問など大きいバンクはバッチ生成され、途中で止めても次回続きから作れる

5. **問題を解く**
  - 作成済みバンクの中から解くものを選び、**言語**（英語問題＋日本語解説／英語のみ／日本語のみ）・**何問解くか**（全問／部分）・**フィードバック**（1問ずつ即解説／まとめて一括採点）を選ぶ
  - **言語はこの解くフェーズで選ぶ**。同じバンクを次回は別の言語で解くこともできる（日本語のみを選ぶと設問・選択肢も日本語に訳して提示される）
  - 採点後にドメイン別の弱点レポートが出る。末尾の推奨に従って、次に作るべきバンク（例「弱点ターゲット10問」）を改めて作成プロセスで作る
