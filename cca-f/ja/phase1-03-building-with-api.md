...
最初の user message を Claude に送信する
Claude のレスポンスを取得し、assistant message として message list に追加する
フォローアップ質問を別の user message として追加する
会話履歴全体を Claude に送信する
...
Tokenization - 入力を小さなまとまりに分割する
Prediction - 次に来る可能性のある単語の確率を計算する
Sampling - それらの確率に基づいて token を選ぶ
...
目標を設定する - プロンプトに何を達成させたいかを定義する
初期プロンプトを書く - 基本的な最初の試みを作成する
プロンプトを評価する - 基準に照らしてテストする
prompt engineering 技法を適用する - 性能を改善するための具体的な方法を使う
再評価する - 変更によって実際に結果が改善したことを確認する
...
劇的な緊張を生む才能を3つブレインストーミングする
最も面白い才能を選ぶ
その才能を明らかにする重要なシーンをアウトライン化する
インパクトを高められる脇役タイプをブレインストーミングする
実際の影響
...
初期リクエスト: 外部ソースから追加データを取得する方法の指示とともに、Claude に質問を送る
Tool Request: Claude が質問を分析し、追加情報が必要だと判断して、必要なデータの具体的な詳細を要求する
データ取得: サーバーがコードを実行し、外部 API やデータベースから要求された情報を取得する
最終レスポンス: 取得したデータを Claude に返し、Claude が元の質問と新しいデータの両方を使って完全なレスポンスを生成する
実践での天気の例
...
tool function code をコピーする
Claude に行き、tool calling 用の JSON schema を書くよう依頼する
tool use に関する Anthropic documentation をコンテキストとして含める
Claude に、ベストプラクティスに従った正しい形式の schema を生成させる
...
tool schema とともに user message を Claude に送る
text block と tool use block を含む assistant message を受け取る
tool information を抽出し、実際の function を実行する
完全な会話履歴とともに tool result を Claude に返す
Claude から最終レスポンスを受け取る
...
ユーザーが尋ねる: "What day is 103 days from today?"
Claude が get_current_datetime を要求する tool use block で応答する
サーバーが function を呼び出し、結果を返す
Claude がさらに情報が必要だと気づき、add_duration_to_datetime を要求する
サーバーがその function を呼び出し、結果を返す
Claude が最終回答を提供するのに十分な情報を得る
...
利用可能な tools とともに user message を Claude に送る
Claude が text や tool requests で応答する
要求されたすべての tools を実行し、result blocks を作成する
tool results を user message として返す
Claude が最終回答を提供するまで繰り返す
...
tool function implementation を作成する
tool schema を定義する
run_conversation の tools list に schema を追加する
run_tool にその tool の case を追加する
...
abstract value 全体が完了するまで待つ
その key-value pair を schema に照らして検証する
abstract 用にバッファリングされたすべての chunks を一度に送信する
meta object について同じ処理を繰り返す
...
既存の main.py ファイルを表示する
円周率計算関数を含む新しい実装で内容を置換する
適切な unit tests を含む新しい test.py ファイルを作成する
なぜテキスト編集ツールを使うのか？
...
テキストを embedding model に入力する
モデルが長い数値リスト（embedding）を出力する
各数値は -1 から +1 の範囲になる
これらの数値は、入力テキストの異なる性質や特徴を表す
数値を理解する
...
テキストをセクションごとに chunk 化する
各 chunk の embeddings を生成する
vector store を作成し、各 embedding を追加する
ユーザーの質問の embedding を生成する
store を検索して最も関連性の高い chunk を見つける
...
prompt を小さな pieces に tokenize する
各 token の embeddings を作成する
周辺テキストに基づいて context を追加する
その後でようやく実際の output text を生成する
...
ファイル（image、PDF、text など）を別の API call で Claude にアップロードする
一意の file ID を含む file metadata object を受け取る
将来の messages で raw file data を含める代わりに、その file ID を参照する
...
Files API を使って data file（CSV など）をアップロードする
message に file ID を持つ container upload block を含める
Claude に data の分析を依頼する
Claude が file を処理する code を書いて実行する
Claude が download 可能な outputs（plots など）を生成できる
実践例
...
Anthropic API key を .env ファイルに追加する
UV（推奨）または pip を使って dependencies をインストールする
starter application を実行して、すべてが動作することを確認する
アプリケーションの実行
...
Tools section に移動する
"List Tools" をクリックして利用可能なすべてのツールを表示する
ツールを選択して testing interface を開く
必要な parameters を入力する
"Run Tool" をクリックして実行し、結果を見る
ドキュメント操作のテスト
...
MCP server code に変更を加える
inspector を通じて個別のツールをテストする
完全な application setup なしで結果を確認する
問題を分離してデバッグする
...
コードが client を使って利用可能なツールを取得する
それらのツールがユーザーの質問とともに Claude に送られる
Claude が read_doc_contents tool を使うと決める
コードが client を使ってそのツールを実行する
結果が Claude に送り返され、Claude がユーザーに応答する
...
autocomplete list に利用可能な resources を表示する
resource を選択できるようにする
resource content を自動的に取得する
その content を Claude への prompt に含める
...
ユーザーが prompt（"format" など）を選択する
システムが必要な arguments（どの document を format するかなど）を求める
補間された values とともに prompt が Claude に送信される
Claude は tools を使って追加 data を取得し、task を完了できる
...
Claude に context を渡す - 前と同じように、関連 files を Claude に見せる
Claude に test cases を考えさせる - 新しい feature を検証する tests を Claude に brainstorm させる
Claude にその tests を実装させる - 最も relevant な tests を選び、Claude に書かせる
tests に合格する code を Claude に書かせる - Claude はすべての tests が通るまで implementation を iterate する
...
image を Claude に渡し、object を describe するよう依頼する
description に基づいて、Claude に CadQuery library を使って object を model するよう依頼する
rendering を作成する
Claude に original image と rendering を比較して grade するよう依頼する。問題があれば修正する
...
同じ image を Claude に同時に複数回送る
各 request には 1 つの material に対する specialized criteria（metal criteria、polymer criteria、ceramic criteria など）を含める
Claude は各 material に対する part の suitability を独立して評価する
すべての analysis results を集め、final aggregation step に渡す
...
Twitter で related trending topics を見つける
最も interesting な topic を選ぶ（Claude を使用）
topic を research する（Claude を使用）
short format video 用の script を書く（Claude を使用）
AI avatar と text-to-speech を使って video を作成する
video を social media に投稿する
...
Step 1: initial prompt を送り、最初の result が完璧でない可能性を受け入れます。Claude は article を生成しますが、一部の constraints に違反しているかもしれません。

Step 2: 問題修正に specifically focused した follow-up request を行います。Claude が書いた article を提供し、targeted revision instructions を与えます。
...
Categorization - ユーザーの topic を Claude に送り、predefined genres の 1 つに分類するよう依頼する
Specialized Processing - category result を使って適切な prompt template を選択し、content を生成する
