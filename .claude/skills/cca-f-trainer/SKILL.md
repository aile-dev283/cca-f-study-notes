...
| 項目 | 確定事項 | 実装 |
|---|---|---|
| 回答形式 | 本番は **multiple-choice と multiple-response の混在**で、各問が選ぶべき数を明示する（Exam Guide 35行） | **本スキルは単一正解4択のみを生成する**（意図的な簡略化であり、公式仕様ではない）。複数選択の練習は本スキルでは行えない点をユーザは理解しておくこと |
| 出題 | シナリオベース。6本中4本がランダム選出され、**全問がシナリオに紐づく** | 6シナリオ原型を内蔵し、問題を必ずシナリオに包む。**どの4本が引かれるか事前に分からないため、演習は6本すべてを被覆する**（不変条件8） |
...
7. **弱点ターゲットは実データに基づく** — `exams/ccar-f/reports/quiz-results/*.json` を集計して優先度を決める。無ければ横断にフォールバックし、その旨を伝える。
   結果ファイルが1つも無い場合は、**「解いたら結果 JSON を `quiz-results/` に保存しないとこの機能は永久に発火しない」ことを明示的に伝える**。
8. **シナリオ被覆を偏らせない** — 生成前に既存全バンクの `scenario_code` を集計し、**未演習（0問）・手薄（5問未満）のシナリオがあれば警告してそこへ厚く配分する**。
   横断で12問以上なら S1〜S6 を最低1問ずつ含める。本番は6本中4本がランダム抽出されるため、演習の薄いシナリオはそのまま失点になる
   （2026-08-08 の不合格はこれが直接原因。詳細は `exams/ccar-f/reports/CCAR-F_ExamAttempt1_Analysis_2026-08-09.md`）。
9. **一般則を無条件化しない** — 解説やディストラクタで「常に X」型の断定をしない。判断ルールは「**条件節 C があるときに限り X、無ければデフォルト D**」の形で書く。
   特に **TS 2.1（ツール説明の拡充）と TS 4.2（few-shot の追加）は「足す」が正解**であり、「足す選択肢を疑え」の一般則で潰してはならない（`antipattern-playbook.md` §3）。
